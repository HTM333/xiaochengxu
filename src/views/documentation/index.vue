<template>
  <div class="app-container">
    <div class="filter-container">
      <!-- <el-input v-model="searchItem" placeholder="名称/手机" style="width: 200px;" class="filter-item" /> -->
      <!-- <el-input
        v-model="searchData"
        placeholder="搜索关键字：昵称、手机、注册名称、管理员"
        style="width: 300px;"
        class="filter-item"
        :clearable="true"
        @clear="searchAll"
   ...
   <el-table-column label="联系电话" align="center">
      ...
      <el-table-column label="关联权限组" align="center">
        <template slot-scope="{row}">
          <span>{{ row.group_id }}</span>
        </template>
      </el-table-column>
      <el-table-column label="关联行业表" align="center">
        <template slot-scope="{row}">
          <span>{{ row.guild }}</span>
        </template>
      </el-table-column> 
      ...
      <el-table-column label="操作" align="center" width="260" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="info"  :disabled="scope.row.aid" style="width:auto"  @click="reset(scope.row.uid)"
          >菜单</el-button>
          <el-button
            type="primary"
            style="width:auto"
          ...
          <el-button size="mini" type="success" style="width:auto" @click="addUpdate(scope.row)">添加收费记录</el-button>
          <el-button
            size="mini"
            style="width:auto"
            type="danger"
    ...
    >
      ...
      >
        ...
        <el-form-item prop="months" label="开通月数:">
          <el-input v-model="temp.months" placeholder="请输入开通月数" />
        </el-form-item>  
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisible = false">取消</el-button>
        <el-button type="primary" @click="shoufeiUpdate()">确认</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        yj_price: [
          { required: true, message: "请输入原价价格", trigger: "blur" }
        ],
        yh_price: [
  ...
  methods: {
    ...
    //获取数据
    getList() {
      workerList(this.listData).then(res => {
        console.log(res);
        if (res.code === 201) {
