    ...
    <el-dialog center :visible.sync="payStatus" title="管理员详情">
      ...
      >
        ...
        <el-form-item style="margin-bottom:0;" label="状态：">
          <el-tag v-if="ruleForm.status===1">正常</el-tag>
          <el-tag v-else type="info">拉黑</el-tag>
        </el-form-item>
        ...
        <el-form-item style="margin-bottom:0;" label="上传时间：">
          <span>{{ruleForm.update_time | parseTime('{y}-{m}-{d}')}}</span>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    checkExist(v) {
      ...
      if (this.temp.username.length <= 20 && this.temp.username.length >= 5) {
        checkUser(d).then(res => {
          console.log(res);
          if (res.code === 201) {
            if (res.errorCode === 0) {
              this.flag = true;
    ...
    //获取数据
    getList() {
      ruleusersList(this.listData).then(res => {
        console.log(res);
        if (res.code === 201) {
